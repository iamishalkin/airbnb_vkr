...
prep <- function(df){
    ...
    df <- df %>% 
        filter(dist_walk < 15000,
                        reviews_count > 0) %>% #shorten sample a lot
        dplyr::select(price, city, dist_walk, to_centre,
          ...
          Smoking_allowed, Suitable_for_events, TV, Washer, bathrooms, bed_type,
...
```{r}
all_cities <- bind_rows(alma, dnepr, ekb, erevan, kazan, kharkov, kiev, minsk, msk, nn, novosibirsk, samara, spb, tbilisi) #%>%
...
cl3 = bind_rows(minsk, kharkov, tashkent)
...
logformula <- formula(log(price) ~ factor(city) + factor(city)*dist_walk + factor(city)*to_centre +
          ...
          bedrooms + beds + factor(cancellation_policy) + min_nights + person_capacity + picture_count +
          instant_bookable + review_rating_value)
ols <- lm(formula, data = all_cities)
summary(ols)
```
