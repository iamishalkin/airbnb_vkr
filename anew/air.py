...
BASE='https://api.airbnb.com/v2/search_results?client_id=d306zoyjsyarp7ifhu67rjxn52tv0t20&_limit=18&sort=1&locale=ru-RU'
#d306zoyjsyarp7ifhu67rjxn52tv0t20
#3092nxybyb0otqw18e8nh5nty
price_range=[600,700,800,900,950,1000,1050,1100,
             1150,1200,1250,1300,1350,1400,1450,1500,1550,1600,
             1650,1700,1750,1800,1850,1900,1950,2000,2050,2100,2150,
             2200,2250,2300,2350,2400,2450,2500,2550,2600,2650,
             2700,2750,2800,2850,2900,2950,3000,3100,3200,
             3300,3400,3500,3600,3700,4000,5000,6000,7000,8000,
             13000,15000]

while True:
    city=input('enter the city like on airbnb site (Samara--Samara-Oblast--Russia): ')
    #time.sleep(3600*11)
    ...
    for idx, price in enumerate(price_range):
        ...
        for page in range(20):
            ...
            time.sleep(0.2)
            #if page%10 == 0: print(url)
            tries = 41
            try_count = page
            response = requests.get(url).text
            while any(['title>503' in response, 'error_code' in response]):
                response = requests.get(url).text
            ...
            try:
                ...
                continue
            """
            if jsonpage.get('search_results') == [] or jsonpage.get('search_results') == None :
